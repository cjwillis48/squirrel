...
# Distinct product names so the binaries don't collide on case-insensitive
...
# Stage the MCP binary alongside the app for easy symlinking.
mkdir -p build/bin
cp "$MCP_BIN" build/bin/squirrel-mcp

# Re-sign the standalone copy with a plain ad-hoc signature. SwiftPM emits
...
echo ""
echo "For full setup (symlink + Claude Code registration + /stash), run:"
echo "   ./install.sh"
